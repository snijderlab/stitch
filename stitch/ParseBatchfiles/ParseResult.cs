...
namespace Stitch {
    ...
    public class RefParseResult<T> {
        /// <summary> The inner value of this RefParseResult, it is UB to use this value if `this.IsErr()`. </summary>
        public T Value = default(T);

        /// <summary> All contained error and warning messages. </summary>
        public readonly List<ErrorMessage> Messages = new();

        /// <summary> Create a RefParseResult with the given value. </summary>
        public RefParseResult(T t) {
            Value = t;
        }

        /// <summary> Create a RefParseResult with the given error message. </summary>
        public RefParseResult(ErrorMessage error) {
            Messages.Add(error);
        }

        /// <summary> Create a RefParseResult with the given error messages. </summary>
        public RefParseResult(List<ErrorMessage> errors) {
            Messages.AddRange(errors);
        }

        /// <summary> Create an empty RefParseResult. </summary>
        public RefParseResult() { }
        ...
        /// <summary> See if this RefParseResult is ok. It will ignore any warnings. </summary>
        public bool IsOk() {
            return !IsErr();
        }

        /// <summary> See if this RefParseResult is ok and store its messages in the given Result. </summary>
        /// <param name="other"> Another RefParseResult to store this ParseResults messages in. </param>
        /// <typeparam name="TOut"> Any Type. </typeparam>
        /// <returns> A bool to indicate if this Result is Ok. </returns>
        public bool IsOk<TOut>(RefParseResult<TOut> other) {
            other.Messages.AddRange(this.Messages);
            foreach (var msg in Messages) {
                if (!msg.Warning) return false;
            }
        ...
        /// <typeparam name="TOut"> Any type. </typeparam>
        /// <returns> The value or default. </returns>
        public T UnwrapOrDefault<TOut>(RefParseResult<TOut> fail, T def) {
            fail.Messages.AddRange(Messages);
            if (this.IsErr()) return def;
            else return this.Value;
        }
        ...
        public RefParseResult<O> Map<O>(Func<T, O> f) {
            var output = new RefParseResult<O>();
            output.Messages.AddRange(Messages);
            if (this.IsOk())
                output.Value = f(this.Value);
            return output;
