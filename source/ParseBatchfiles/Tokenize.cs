...
namespace AssemblyNameSpace
{
    ...
    {
        public static List<ParseCommandFile.KeyValue> Tokenize(string content)
        {
            var parsed = new List<ParseCommandFile.KeyValue>();
        ...
        static class TokenizeHelper
        {
            ...
            public static (ParseCommandFile.KeyValue, string) MainArgument(string content)
            {
            ...
            /// </summary>
            /// <returns>The status</returns>
            static (ParseCommandFile.KeyValue, string) Argument(string content)
            {
            ...
            /// <returns>The status</returns>
            static (ParseCommandFile.KeyValue, string) SingleParameter(string content, string name)
            {
                content = content.Remove(0, 1).Trim();
                //Get the single value of the parameter
                string value = ParseHelper.Value(ref content);
                return (new ParseCommandFile.KeyValue(name, value), content);
            }
            /// <summary>
            ...
            /// <param name="name">The name of the parameter</param>
            /// <returns>The status</returns>
            static (ParseCommandFile.KeyValue, string) MultilineSingleParameter(string content, string name)
            {
                content = content.Remove(0, 2).Trim();
                //Get the single value of the parameter
                string value = ParseHelper.UntilSequence(ref content, "<:");
                return (new ParseCommandFile.KeyValue(name, value.Trim()), content);
            }
            /// <summary>
            /// Multiparameter
            /// </summary>
            ...
            /// <param name="name">The name of the parameter</param>
            /// <returns>The status</returns>
            static (ParseCommandFile.KeyValue, string) MultiParameter(string content, string name)
            {
                content = content.Remove(0, 2).Trim();
                // Now get the multiple values
                var values = new List<ParseCommandFile.KeyValue>();
                ...
                while (true)
                {
                    if (content[0] == '<' && content[1] == '-')
                    {
                        ...
                        content = content.Remove(0, 2).Trim();
                        return (new ParseCommandFile.KeyValue(name, values), content);
                    }
                    else
                    {
